...
namespace ISAAR.MSolve.FEM.Embedding
{
    public class EmbeddedBeam3DGrouping
    {
        ...
        private readonly Element[]  hostGroup;
        private readonly Element[] embeddedGroup;
        ...
        public static EmbeddedBeam3DGrouping CreateFullyBonded(Model  model, Element[] hostGroup, Element[] embeddedGroup, bool hasEmbeddedRotations)
        {
            return new EmbeddedBeam3DGrouping(model, hostGroup, embeddedGroup, hasEmbeddedRotations, 0);
        }

        public static EmbeddedBeam3DGrouping CreateCohesive(Model  model, Element[] hostGroup, Element[] embeddedGroup, bool hasEmbeddedRotations)
        {
            return new EmbeddedBeam3DGrouping(model, hostGroup, embeddedGroup, hasEmbeddedRotations, 2);
        }

        private EmbeddedBeam3DGrouping(Model  model, Element[] hostGroup, Element[] embeddedGroup, bool hasEmbeddedRotations, int skip)
        {
            this.model = model;
            ...
            foreach (var embeddedElement in embeddedGroup)
            {
                ...
                foreach (var node in embeddedElement.Nodes.Skip(skip))
                {
                    ...
                    foreach (var embeddedNode in embeddedNodes)
                    {
                        ...
                        foreach (var element in hostGroup)
                            if (element.ElementType is IEmbeddedElement currentElementType && 
                                element.Nodes.Contains(embeddedNode.Node) &&
                                !currentElementType.EmbeddedNodes.Contains(embeddedNode))
                            {
