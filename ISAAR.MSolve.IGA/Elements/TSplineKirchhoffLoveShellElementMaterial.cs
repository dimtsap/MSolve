    ...
    public class TSplineKirchhoffLoveShellElementMaterial: Element, IStructuralIsogeometricElement
	{
		...
		public Tuple<double[], double[]> CalculateStresses(Element element, double[] localDisplacements, double[] localdDisplacements)
		{
			...
			for (int j = 0; j < materialsAtThicknessGP.Keys.Count; j++)
			{
				...
				var membraneStrain = Bmembrane * new Vector(localDisplacements);
				var bendingStrain = Bbending * (new Vector(localDisplacements));
				
				foreach (var keyValuePair in materialsAtThicknessGP[materialsAtThicknessGP.Keys.ToList()[j]]) // for each key value pair sto mesa Dictionary dld
				{
		...
		{
            //TODO: (for m in all the materials m.ClearState) throws an exception. Investigate. Possible changes in Analyzers may be the cause.
        }
			...
			IList<GaussLegendrePoint3D> gaussPoints, int j)
		{
			...
			foreach (var keyValuePair in materialsAtThicknessGP[gaussPoints[j]])
			{
                ...
                gpForces.Scale(thicknessPoint.WeightFactor); //todo scale se diaforetiko vector gia na mhn peiraxthoun oi stresses
