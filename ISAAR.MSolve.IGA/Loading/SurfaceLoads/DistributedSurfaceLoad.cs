           ...
           var loadTable = new Table<INode, IDofType, double>();
			...
			for (int gp = 0; gp < integration.IntegrationPoints.Count; gp++)
			{
				...
				for (int indexNode = 0; indexNode < nodes.Count; indexNode++)
				{
					...
					var valueX = _loadX * shapeFunctionNatural[indexNode,gp] * jacdet *
								 weightFactor;
					var valueY = _loadY * shapeFunctionNatural[indexNode,gp] * jacdet *
								 weightFactor;
					var valueZ = _loadZ * shapeFunctionNatural[indexNode,gp] * jacdet *
								 weightFactor;
