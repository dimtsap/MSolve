...
{
    public class ProximityDensityFilterIGA2D : IDensityFilter
    {
        ...
        /// </param>
        public ProximityDensityFilterIGA2D(Model model, double filterAreaRadius)
        {
            this.filterAreaRadius = filterAreaRadius;

            // Cast the continuum elements. //TODO: this process is already performed in LinearAnalysis2DGeneral.
        ...
        private ElementNeighborhood FindNeighbors(double radius, ContinuumElement2D[] allElements, 
            int elementIdx)
        {
            ...
            CartesianPoint thisCentroid = allElements[elementIdx].FindCentroid(modelElements[elementIdx]);
            for (int f = 0; f < allElements.Length; ++f)
            {
                CartesianPoint otherCentroid = allElements[f].FindCentroid(modelElements[elementIdx]);
